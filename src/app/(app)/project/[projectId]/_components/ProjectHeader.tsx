...
export function ProjectHeader({
  ...
  const updateProj = api.project.update.useMutation({
    onSuccess: (data) => {
      setName(data.name);
      setIsEditing(false);
      router.refresh();
  ...
  const handleSave = () => {
    ...
    if (!trimmed || trimmed === name || updateProj.isPending) {
      setIsEditing(false);
      return;
    }
    updateProj.mutate({ projectId, name: trimmed });
  };
      ...
      <div style={{ flex: 1 }}>
          ...
          {isEditing ? (
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <button
                ...
                disabled={!inputVal.trim() || updateProj.isPending}
        ...
        {updateProj.error && (
          <div className="alert alert-error" style={{ marginTop: 12, marginBottom: 0 }}>
            {updateProj.error.message}
          </div>
        )}
      </div>

      {!isSettingsTab && isProjectManager && <ImportButton projectId={projectId} />}
    </div>
  );
}
